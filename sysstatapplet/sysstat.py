...
class Preferences(QDialog):
    def __init__(self, sysstat):
        ...
        verbose = QCheckBox("")
        verbose.toggled.connect(lambda checked: self.sysstat.verbose.setValue(checked))
...
class SysStat(Indicator):
    ...
    def initVars(self):
        syslog.syslog(syslog.LOG_DEBUG, "DEBUG  %s: initializing variables" % self.name)

        # external command
        self.extcmd = SettingsValue("%s/extcmd" % self.name, self.extcmd_default, str)
    ...
    def initContextMenu(self):
        syslog.syslog(
            syslog.LOG_DEBUG, "DEBUG  %s: initializing context menu" % self.name
        )
        m = QMenu()
        m.addAction(QIcon.fromTheme("view-refresh"), "&Reload", lambda: self.restart())
        m.addAction(
            QIcon.fromTheme("preferences-other"), "&Settings", lambda: self.prefs.show()
        )
        m.addAction(
    ...
    def initWidgets(self):
        syslog.syslog(
            syslog.LOG_DEBUG, "DEBUG  %s: initializing preferences" % self.name
        )
    ...
    def runExternalCmd(self):
        syslog.syslog(
            syslog.LOG_DEBUG, "DEBUG  %s: running external command" % self.name
        )
    ...
    def systrayClicked(self, reason):
        ...
        if (
            ...
            if self.verbose.value():
                syslog.syslog(
                    syslog.LOG_DEBUG, "DEBUG  %s: QSystemTrayIcon::Context" % self.name
                )
        elif reason == QSystemTrayIcon.ActivationReason.Unknown:
            syslog.syslog(
                syslog.LOG_DEBUG, "DEBUG  %s: QSystemTrayIcon::Unknown" % self.name
            )
