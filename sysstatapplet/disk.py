...
from appletlib.app import SettingsValue
...
class SplashDisk(Splash):
    ...
    def paintEvent(self, ev):
        ...
        for k in sorted(self.data.keys()):
            ...
            p.drawText(xpos, 0, self.br1.width(), lh, Qt.AlignmentFlag.AlignLeft, k)
            xpos += self.br1.width() + self.margin
            p.setPen(self.indicator.systray.fgColor)
            p.drawText(
                xpos, 0, self.br1.width(), lh, Qt.AlignmentFlag.AlignRight, "read"
            )
            xpos += self.br1.width() + self.margin
            p.setPen(QColorConstants.Green)
            p.drawText(
                xpos,
                0,
                self.br2.width(),
            ...
            p.setPen(self.indicator.systray.fgColor)
            p.drawText(
                xpos, 0, self.br1.width(), lh, Qt.AlignmentFlag.AlignRight, "write"
            )
            xpos += self.br1.width() + self.margin
        ...
        if len(self.data) > 1:
            xpos = 0
            p.setPen(QColorConstants.White)
            p.drawText(xpos, 0, self.br1.width(), lh, Qt.AlignmentFlag.AlignLeft, "all")
            xpos += self.br1.width() + self.margin
            p.setPen(self.indicator.systray.fgColor)
            p.drawText(
                xpos, 0, self.br1.width(), lh, Qt.AlignmentFlag.AlignRight, "read"
...
class IndicatorDisk(SysStat):
    def __init__(self):
        ...
        self.splash.triggerResize.connect(lambda ev: self.updateSplashGeometry())
        self.addPrefs()

    def initVars(self):
        SysStat.initVars(self)
        self.ignoreList = SettingsValue("%s/ignore" % self.name, [], "QStringList")
        self.old = {}
        self.new = {}
    ...
    def addPrefs(self):
        ...
        ignoreListWid.returnPressed.connect(
            lambda: self.ignoreList.setValue(
                [
                    regex.strip()
                    for regex in ignoreListWid.text().split(",")
                    if len(regex.strip())
                ]
            )
        )
        ...
        if n > 0:
            ...
            for k in sorted(self.new.keys()):
                ...
                if k in self.old:
                    ...
                    if read:
                        p.fillRect(
                            w - 4, round(bh * 0.5) - 4, 4, 4, QColorConstants.Green
                        )
                    if written:
                        p.fillRect(w - 4, round(bh * 0.5), 4, 4, QColorConstants.Red)
                else:
                    data[k] = (0, 0)
