...
class SplashCpu(Splash):
    ...
    def paintEvent(self, ev):
        ...
        for k in sorted(self.data, key=lambda i: sum(i[2:]), reverse=True)[:5]:
            ...
            p.drawText(
                xpos, 0, self.br1.width(), lh, Qt.AlignmentFlag.AlignRight, str(k[0])
            )
            xpos += self.br1.width() + self.margin
            p.setPen(self.indicator.systray.fgColor)
            p.drawText(xpos, 0, 100, lh, Qt.AlignmentFlag.AlignRight, k[1][1:-1])
...
class IndicatorCpu(SysStat):
        ...
        with open("/proc/stat") as f:
            ...
            for line in f:
                if not line.startswith("cpu"):
                    break
                l = line.split()
                new[l[0]] = (l[1], l[2], l[3], l[4], sum([int(i) for i in l[1:]]))
            if len(self.old):
    ...
    def update(self):
        ...
        total = 0
        for k in keys:
            user = sum([int(i) for i in self.new[k][0:1]]) - sum(
                [int(i) for i in self.old[k][0:1]]
            )
            ...
            p.fillRect(
                0, 0, round(box.height() * perc2), box.width(), self.systray.fgColor
            )
            c2 = QColor(self.systray.fgColor)
            c2.setAlphaF(0.25)
            p.fillRect(
                round(box.height() * perc2),
                0,
                round(box.height() * perc1),
                box.width(),
                c2,
            )
        ...
        work = sum([int(i) for i in self.new["cpu"][0:2]]) - sum(
            [int(i) for i in self.old["cpu"][0:2]]
        )
